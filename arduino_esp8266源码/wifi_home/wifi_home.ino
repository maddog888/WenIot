...
#include <ArduinoJson.h>   //Json模块库文件

//************在下面区域添加即可*****************
//外置模块
#include "DHT.h"           //DHT模块库文件
...
const uint16_t mqtt_broker_port = 8083;          // 服务端口号

const char *mqtt_username = "********";  // 账号（非必须）
const char *mqtt_password = "********";  // 密码（非必须）

const uint16_t mqtt_client_buff_size = 4096;  // 客户端缓存大小（非必须）
String mqtt_client_id = "wen1_client";       // 客户端ID

const char *mqtt_topic_pub = "q1";  // 需要发布到的主题，用户控制端订阅
const char *mqtt_topic_sub = "q2";  // 需要订阅的主题，用户控制端发布
...
//以下设置需要用到的针脚
DHT dht(D1, DHT11); //温湿传感器
const int led1 = D2;  //开关
const int fishLed = D5;  //喂鱼警告
const int fishOn = D6;  //关闭警告
//************在上面区域添加即可*****************
...
void setup() {
  ...
  ********************************************/
  ...
  //初始化针脚
  pinMode(led1, OUTPUT);  //房间开关
  pinMode(fishLed, OUTPUT);  //警报开关
  pinMode(fishOn, INPUT);  //警报开关
...
void loop() {
  ...
  if(digitalRead(fishOn)==1){
    delay(5);
    if(digitalRead(fishOn)==1){
      if(digitalRead(fishLed)){
        startDTimer(172800000,2);  //重新添加一个倒计时
        fishOF("0");
        delay(200);
        fishOF("1");
        delay(500);
        fishOF("0");
        while(digitalRead(fishOn)==1);
      }
    }
  }
...
//喂鱼警告开关
void fishOF(String value){
  digitalWrite(fishLed, value.toInt());
}


//开关a
void ledOF(String value){
  //如果是请求打开
  if(value=="1"){
...
void sendUpdate(){
  ...
  // 向 JSON 对象添加数据
  doc["led1"] = (digitalRead(led1) ? 0 : 1); //获取相反状态是0就传1，是1就传0
  doc["fishLed"] = (digitalRead(fishLed) ? 1 : 0);

  doc["temperature"] = String(t, 0) + "°C"; //赋值
  doc["humidity"] = String(h, 0) + "%";  //赋值
